...
impl GeneratedMerkleTreeCollection {
    pub fn new_from_stake_meta_collection_with_config(
        ...
        let pool_stake_account_of_interest = get_pool_stake_info(
            ...
            distribution_config.stake_pool_ids,
        );
        ...
        let generated_merkle_trees = stake_meta_coll
            ...
            .filter_map(|stake_meta| {
                ...
                let mut tree_nodes = match TreeNode::vec_from_stake_meta_with_config(
                    &stake_meta,
...
impl TreeNode {
    ...
    pub fn vec_from_stake_meta_with_config(
        ...
        if let Some(reward_distribution_meta) = stake_meta.maybe_reward_distribution_meta.as_ref() {
            ...
            // Start with the base number of delegations
            let mut claim_transactions = stake_meta.delegations.len();
            // Count additional claimants from the config
            ...
            if let Some(cfg) = validator_config.as_ref() {
                ...
                if let Some(_split) = &cfg.validator_reward_split {
                    // If there's a validator split, it adds one more claimant
                    // (the validator is already counted by default)
                    claim_transactions += 1;
                }
            }

            // Add 1 for the main validator and 1 for the expense node
            let total_claimants = claim_transactions + 2;

            let close_claim_status_and_rda_transactions =
                (total_claimants as f64 / CLOSE_INSTR_PER_TXN as f64).ceil() as usize + 1;
            let total_transactions = 1 + total_claimants + close_claim_status_and_rda_transactions;
            let expense_amount = (total_transactions * 5000) as u64;
            ...
            let validator_commission_bps = reward_distribution_meta.validator_fee_bps as u128;
            ...
            if reward_distribution_meta.total_rewards > total_distributed {
                ...
                if let Some(validator_node) = tree_nodes
                    .iter_mut()
                    .find(|node| node.claimant == validator_identity_account)
                {
                    validator_node.amount = validator_node
                        .amount
                        .checked_add(undistributed_amount)
                        .ok_or(MerkleRootGeneratorError::MathOverflow)?;
                }
                warn!(
                    "distributed_amount_less: total_rewards_to_distribute={},total_distributed={},undistributed_amount={} claimed to validator",
